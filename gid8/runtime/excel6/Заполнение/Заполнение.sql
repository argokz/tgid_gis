...
--  lbf.heatSourceID, lbf.monthID, 
  ...
  lbf.netTemperature, lsm.tx,
  
  (SELECT Gmag)*(lbf.netTemperature-lsm.tx)*1E-3,
  (SELECT Grs)*(lbf.netTemperature-lsm.tx)*1E-3 AS Qrs,
  (SELECT Gtep)*(lbf.netTemperature-lsm.tx)*1E-3 AS Qtep
...
LEFT JOIN heatLosesSourceMonths lsm ON lbf.heatSourceID = lsm.heatSourceID AND lbf.monthID=lsm.m AND lsm.sezon=2
