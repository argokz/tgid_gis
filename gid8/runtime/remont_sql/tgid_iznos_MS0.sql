...
--      iz.id,
    ...
    --  "??" AS procent, -- Процент износа, % 
      ...
      IIF(mag.naimenovanie_magistrali IS NULL, rs.naimenovanie_uchastka_rs,mag.naimenovanie_magistrali) AS mag, -- Магистраль/РаспредСеть
      tt.name AS name_tt,   -- Тип прокладки
      tt2.name AS name_tt2, -- Материал трубы

      hps1.diameterCondit, -- Условный  диаметр трубопровода, мм
      IIF(_T2.lenP >= _T2.lenO, IIF (_T2.lenP > 0, _T2.lenP, 1), _T2.lenO) AS len, -- Протяженность участка, м
      ...
      DATEDIFF(YEAR,hps1.firstPICdateHP,(SELECT date_calc)) AS srok2,  -- Срок эксплуатации, лет
...
--      iz.SostOborudovania AS v20, -- Состояние оборудования       1 - новое, почти новое  2 - в работе в неаварийном состоянии 3 в работе в предаварийном состоянии 4 в работе в аварийном состонии 5 не работает вес  0.2
      ...
      IIF(cap1.n_defekt IS NULL, 0, cap1.n_defekt) AS n_cap, -- Количество капитальных ремонтов   - из точек
      IIF(df1.n_defekt IS NULL, 0, df1.n_defekt) AS n_def, -- Количество нарушений (после последней перекладки) 
...
--      iz.SostKonstrukz AS v27, -- Состояние строительных конструкций  1 Отклонение от проектных решений, произошедшие в процессе эксплуатации  3 - Снижение несущей способности конструкций  3 Нарушение герметичности стыков  5 -  Излом конструкций перекрытия и стен канала  5 Разрушение неподвижных опор  вес 0.1
    ...
    -- 1-5
      ...
      IIF((SELECT srok2) <= 7, 1, IIF((SELECT srok2) <= 15, 2, IIF((SELECT srok2) < 20, 3, IIF((SELECT srok2) < 25, 4, 5)))) AS srok2_ball,  --   вес 0.15
      IIF((SELECT v24) < 1, 1, IIF( (SELECT v24) < 2,  2, IIF( (SELECT v24) < 4, 3, IIF((SELECT v24) < 5, 4, 5)))) AS koef_povr, -- 0.2
      IIF(pps.VnesniiVid IS NULL, 0, pps.VnesniiVid) AS VnesniiVid2,
      IIF(pps.SostOborudovania IS NULL, 0, pps.SostOborudovania) AS SostOborudovania2,
      IIF(pps.Kategorii IS NULL, 0, pps.Kategorii) AS Kategorii2,
      IIF(pps.VnesnKorrozia IS NULL, 0, pps.VnesnKorrozia) AS VnesnKorrozia2,
      IIF(pps.VnunrenKorrozia IS NULL, 0, pps.VnunrenKorrozia) AS VnunrenKorrozia2,
      IIF(pps.SostKonstrukz IS NULL, 0, pps.SostKonstrukz) AS SostKonstrukz2,
    ...
    SELECT 
      hps2.pipeSectionID, 
      COUNT(hps2.id) cnt,
      SUM(IIF(l.externalSignLineID IN (1,2,4), hps2.pipeSectLength, 0)) AS lenP,
      SUM(IIF(l.externalSignLineID IN (1,3,5), hps2.pipeSectLength, 0)) AS lenO 
    ...
    LEFT JOIN magistrali mag ON mag.id=hps1.magistral
    LEFT JOIN uchastok_rs rs ON rs.id=hps1.distSite
    ...
    LEFT JOIN iznos iz ON iz.pipeSectionID=pps.id
    LEFT JOIN CALCULATION_IZNOS calc ON calc.id=iz.calculationID --  AND calc.DateRascheta='$DATE$'
