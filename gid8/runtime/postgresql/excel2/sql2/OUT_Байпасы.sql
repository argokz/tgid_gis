...
--  eci.name as kod_p, ni.externalNodeName as uzel_p, esi.name as pr_p,
  ...
  case bp_out.externalSignLineID    when 1 then ' '    when 2 then 'Ï'    when 3 then 'Î'    when 4 then 'Ï'    when 5 then 'Î'  end as pr1,
  a4,
  a5, 

  ec2.name as kod2, n2.externalNodeName as uzel2,
  case bp_out.externalSignLineID    when 1 then ' '    when 2 then 'Ï'    when 3 then 'Î'    when 4 then 'Î'    when 5 then 'Ï'  end as pr2,
  a9, 
  a10, 
...
--  , sopr
from BP_OUT
join linesobj l on l.id = bp_out.lineID
...
join externalSignLine esl on esl.id = bp_out.externalSignLineID
...
WHERE n1.fileID=$fileID$ AND calculationID=$calculationID$ AND l.removed=0
