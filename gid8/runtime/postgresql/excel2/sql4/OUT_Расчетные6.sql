...
hs.name AS name_src,
  ...
  sum(isnull(Qotz_treb,0)+isnull(Qotn_treb,0)+isnull(Qvent_treb,0)+isnull(Qgvz_treb,0)) AS Qz_treb_,
  sum(G_gz) AS G_gz_,
  ...
  sum(Qras_gv_z+otopl_nco+otopl_zco+ventil_cv+kondiz) AS gz_r,
  sum(G_gz) AS G_gz_r,
  sum(gv_op) AS gvop_r,
