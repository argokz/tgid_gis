SELECT name, val 
FROM (
  SELECT 1 AS id1, id, 'Координаты точки начала рабочей зоны, расход' AS name, q_min AS val FROM standardPumps
  UNION SELECT 2, id, 'Координаты точки начала рабочей зоны, напор', h_min FROM standardPumps
  UNION SELECT 3, id, 'Координаты точки конца рабочей зоны, расход', q_max FROM standardPumps
  UNION SELECT 4, id, 'Координаты точки конца рабочей зоны, напор', h_max FROM standardPumps
  UNION SELECT 5, id, 'Расход воды через насос номинальный', q_nomin FROM standardPumps
  UNION SELECT 6, id, 'Напор воды через насос номинальный', h_nomin FROM standardPumps
  ...
  UNION SELECT 12,id, 'Давление воды на всасе насоса',p_home  FROM standardPumps
  ...
  UNION SELECT 14,id, 'Давление рабочее на входе максимальное',p_home_max  FROM standardPumps
