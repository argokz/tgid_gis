DROP TABLE IF EXISTS __InitialJoin;


CREATE TEMP TABLE __InitialJoin AS 
SELECT
    DISTINCT
    l.lineID,
    ...
    FROM (
        SELECT
            DISTINCT
                ...
                d.data_osmotra,
--                l.organizationID,
                ST_Distance(l.shape, ST_SetSRID(d.shape, 910000)) as length,
                MIN(ST_Distance(l.shape, ST_SetSRID(d.shape, 910000))) OVER(PARTITION BY d.id ) AS "min_len"
        FROM defect d
        LEFT JOIN linesobj l ON ( l.removed = 0 and ST_Distance(l.shape, ST_SetSRID(d.shape, 910000)) < 0.1 )
...
--            hpss.pipeSectLength as "Длина участка теплопровода, м",
            ...
            deft.name as "Режим",
            t.data_osmotra as "Дата обнаружения нарушения",
