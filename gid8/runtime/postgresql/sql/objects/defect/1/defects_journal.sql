            ...
            from (
                select
                    distinct
                        ...
                        d.stateID,
                        l.organizationID,
                        ST_Distance(l.shape, d.shape) as length,
                        MIN(ST_Distance(l.shape, d.shape)) OVER(PARTITION BY d.id ) AS "min_len"
                from defect d
                JOIN linesobj l ON ( l.removed = 0 and ST_Distance(l.shape, d.shape) < 0.1 )
                JOIN nodes n1 ON n1.id=l.nodeID1 AND n1.removed=0 $and_n1_fileID$
        )k
