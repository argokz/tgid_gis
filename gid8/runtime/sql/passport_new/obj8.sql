...
STUFF (
(
select concat(', ', el2.name)  AS name 
from prichinyPovrezhdenia el2
JOIN prichinyPovrezhdeniaForDefect el1 ON el2.id = el1.activityID AND el1.objID=d.objID
FOR XML PATH(''), TYPE
).value('.', 'NVARCHAR(MAX)')
, 1, 1, '') AS v

FROM prichinyPovrezhdeniaForDefect d
