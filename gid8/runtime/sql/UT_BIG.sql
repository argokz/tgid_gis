...
MAX(hps.diameterCondit) over (partition by pss.id) AS 'Условный диаметр, мм',
MAX(hps.diameterExternal) over (partition by pss.id) AS 'Наружный диаметр, мм',
MAX(hps.wallThickness) over (partition by pss.id) AS 'Толщина стенки, мм',
