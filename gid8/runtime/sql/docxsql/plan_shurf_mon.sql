...
select distinct top 1000000
    ...
    CONCAT(CONVERT(varchar, t.data_nachala_plan, 105), '-', CONVERT(varchar, t.data_okonchaniya_plan, 105)) as 'Дата шурфовки',
    
    t.data_nachala_plan as 'Дата начала шурфовки',
    t.data_okonchaniya_plan as 'Дата окончания шурфовки',
    ...
    DAY(t.data_utverzhdeniya_plana_shurfovok) as 'День утверждения плана шурфовок',
    YEAR(t.data_utverzhdeniya_plana_shurfovok) as 'Год утверждения плана шурфовок',
    MONTH(t.data_utverzhdeniya_plana_shurfovok) as 'Месяц утверждения плана шурфовок',
            ...
            from (
                select
                    distinct
                        ...
                        l.externalSignLineID,
                        l.organizationID,
                        ROW_NUMBER() OVER (PARTITION BY d.id ORDER  BY d.shape.STDistance(l.shape)) AS rn
                from shurfy d
                left JOIN linesobj l ON ( l.removed = 0 and l.shape.STDistance(d.shape) < 0.1 )
                left join nodes n1 ON n1.id = l.nodeID1
                where (d.data_nachala_plan $season_condition$)  $and_naznachenie_vskrID$
                $and_name_date_mon$  $and_n1_fileID$
                --and (d.data_nachala_plan  BETWEEN '20220415' AND '20230415' )
            )k
        where k.rn = 1
    ) l on l.obj_id = d.id  $and_naznachenie_vskrID$
