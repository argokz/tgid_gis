...
SELECT
		...
		ps1.pipeLength,
		tt.name AS typ_pr,
		IIF(faktory_riska_truboprovoda.id IS NULL,'не осмотрено', 'осмотрено') as statusOsmotr 
, srt.orderID
	FROM $opres$ obj
	JOIN $opres$Deployed d ON d.directionID=obj.id
		...
		left join faktory_riska_truboprovoda on faktory_riska_truboprovoda.lineID = ps1.id and faktory_riska_truboprovoda.objID = $m_opr_id$ and faktory_riska_truboprovoda.obj_type_faktory_riskaID = $obj_type_faktory_riskaID$
