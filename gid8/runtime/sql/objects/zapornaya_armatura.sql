...
SELECT  distinct
        ...
        damperArmatureStates.name AS 'Состояние задвижки',


        IIF (n0.nodeName is NULL or n0.nodeName = '' or n0.nodeName = ' ',n0.externalNodeName, n0.nodeName) as 'Камера включения',

        obj.nomer2 AS 'Номер задвижки',

        obj.primechanie AS 'Примечание 1',
        obj.nomer AS 'Примечание 2',

        obj.prinadlezhnost as 'Принадлежность',
        IIF(MIN(obj.shape.STDistance(n1.shape)) OVER(PARTITION BY obj.id ) < MIN(obj.shape.STDistance(n2.shape)) OVER(PARTITION BY obj.id ), IIF (n1.nodeName is NULL or n1.nodeName = '' or n1.nodeName = ' ',n1.externalNodeName, n1.nodeName), IIF (n2.nodeName is NULL or n2.nodeName = '' or n2.nodeName = ' ',n2.externalNodeName, n2.nodeName)) as 'Наименование узла/павильона',
--        NULL as 'Наименование узла/павильона',
        loc.name as 'Месторасположение',
        obj.god_vvoda as 'Год ввода',
...
--        obj.primechanie as 'Примечание',
    ...
    IIF (nu_ms.fio is not NULL, nu_ms.fio, nu_rs.fio) as 'Начальник участка',
        pss.id as ps_id,
--        srt.orderID srt_orderID,
        MAX(srt.orderID) over ( partition by obj.id) as srt_orderID,
    ...
    SELECT distinct
        ...
        obj.id
    FROM zapornaya_armatura obj     
    LEFT JOIN linesobj l ON l.removed = 0 and l.shape.STDistance(obj.shape) < 0.3
...
--    LEFT JOIN Locations loc ON loc.id = obj.mestoraspolozhenie
    LEFT JOIN locationTypes loc ON loc.id = obj.locationTypesID
    ...
    LEFT JOIN nodes n0 ON n0.id = obj.nodeID_pts
    ...
    left join damperArmatureStates ON damperArmatureStates.id = obj.stateID


    WHERE 
    pss.id is not null $AND_N1FILEID$
