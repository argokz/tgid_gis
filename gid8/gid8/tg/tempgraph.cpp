...
const std::map<int, QString> map_help = {
{ 500, "Ошибки в наборе данных!" },
...
{ 567, "Расчетная температура в помещении меньше температуры конца сезона" },
{ 568, "Расчетные температуры трубопроводов заданы некорректно" },
{ 569, "Температура верхней срезки должна быть больше температуры нижней" },
{ 570, "Конфликт температур узла смешения и обратного трубопровода" },
{ 571, "Конфликт температур узла смешения и подающего трубопровода" },
{ 572, "Расчетная температура наружного воздуха должна быть отрицательной" },
{ 573, "Расчетная температура наружного воздуха не должна быть менее -65" },
{ 574, "Расчетная температура воздуха внутри помещения меньше наружной" },
{ 575, "Температура подающего трубопровода должна быть положительной" },
{ 576, "Температура обратного трубопровода должна быть положительной" },
{ 577, "Температура нижней срезки графика должна быть положительной" },
{ 578, "Расчетная температура наружного воздуха выше температуры вохдуза в конце сезона" },
{ 579, "Температура горячей воды должна быть положительной" },
{ 580, "Конфликт температур горячей воды и подающего трубопровода" },
{ 581, "Температура холодной воды должна быть больше 0" },
{ 582, "Температура холодной воды не должна быть больше 20°C" },
{ 583, "Требуемая температура воздуха внутри помещения меньше наружной" },
{ 584, "Относительная погрешность расчета должна быть положительной" },
{ 585, "Максимальная относительная погрешность расчета больше 0.1" },
{ 586, "Относительный напор на ТЭЦ меньше 1" },
{ 587, "Дефицит мощности источника" },
{ 588, "Расчетная нагрузка отопления < 0" },
{ 589, "Расчетная нагрузка горячего водоснабжения < 0" },
{ 590, "Фактический коэффициент смещения <= 0" },
{ 591, "Сумма относительных сопротивлений не равна 1" }
};
...
void InitOtoplTempStructure(QSqlQuery &query, COMMON_TEMP_STRUCTURE& tempstruct, COMMON_TEMP_STRUCTURE& tempstruct_norm)
{
    tempstruct.feat = Otop;
    delete tempstruct.ot;
    tempstruct.ot = new OTOP();

    tempstruct.ot->THOR = query.value("tn_5").toDouble(); // Расчетная температура наружного воздуха
    tempstruct.ot->THK = query.value("tn_1").toDouble(); // Температура наружного воздуха конца отопительного сезона
    tempstruct.ot->TVR = query.value("tvn_r").toDouble(); // Расчетная температура воздуха внутри помещения

    tempstruct.ot->TAURP = query.value("t1_r").toDouble();  // Расчетная темп-ра сетевой воды в подающем трубопроводе
    tempstruct.ot->TAURO = query.value("t2_r").toDouble();  //      -"-                       в обратном
    tempstruct.ot->TAURS = query.value("t3_r").toDouble();  //      -"-                       после узла смешения

    tempstruct.ot->QR = query.value("q_r").toDouble();  // Расчетная тепловая нагузка на отопление
    tempstruct.ot->TSMIN = query.value("t1_2r").toDouble();  // Температура нижней срезки  сетевой воды

    tempstruct.ot->TSMAX = query.value("t1_4r").toDouble();  // Температура верхней срезки  сетевой воды
    tempstruct.ot->T2MIN = query.value("t2_2r").toDouble();  // Температура нижней срезки обратной воды
    ...
    tempstruct.ot->KSR = query.value("uf").toDouble();  // Поправка коэффициента смешения
    tempstruct.ot->V = query.value("v").toDouble();  // Скорость ветра
    tempstruct.ot->TB = query.value("tvb_tr").toDouble();  // Требуемая температура воздуха внутри отап-х помещениях
...
*/
    ...
    tempstruct.ot->QMAX = query.value("hSourcePower").toDouble();
...
void InitPovTempStructure(QSqlQuery &query, COMMON_TEMP_STRUCTURE& tempstruct, COMMON_TEMP_STRUCTURE& tempstruct_norm)
{
    tempstruct.feat = Pov;
    delete tempstruct.pov;
    tempstruct.pov = new POV();

    tempstruct.pov->THOR = query.value("tn_5").toDouble(); // Расчетная температура наружного воздуха
    tempstruct.pov->THK = query.value("tn_1").toDouble(); // Температура наружного воздуха конца отопительного сезона
    tempstruct.pov->TVR = query.value("tvn_r").toDouble(); // Расчетная температура воздуха внутри помещения

    tempstruct.pov->TAURP = query.value("t1_r").toDouble();  // Расчетная темп-ра сетевой воды в подающем трубопроводе
    tempstruct.pov->TAURO = query.value("t2_r").toDouble();  //      -"-                       в обратном
    tempstruct.pov->TAURS = query.value("t3_r").toDouble();  //      -"-                       после узла смешения

    tempstruct.pov->QOR = query.value("q_r").toDouble();  // Расчетная тепловая нагузка на отопление
    tempstruct.pov->QGW = query.value("q_gv").toDouble();   // Среднечасовая нагрузка горячего водоснабжения, Гкал/ч
    tempstruct.pov->TSMIN = query.value("t1_2r").toDouble();  // Температура нижней срезки  сетевой воды

    tempstruct.pov->TSMIN = 0; // Для повышенного графика не бывает нижней срезки, сказала Надежда Ивановна (2022.01.19)
                              // Манюк стр 155,166
    
    
    tempstruct.pov->TSMAX = query.value("t1_4r").toDouble();  // Температура верхней срезки  сетевой воды

    tempstruct.pov->TVRO = query.value("tg_r").toDouble();  // Температура горячей воды в точке водоразбора
    tempstruct.pov->TV = query.value("tx_r").toDouble();  // Температура холодной воды
    tempstruct.pov->TB = query.value("tvb_tr").toDouble();  // Требуемая температура воздуха внутри отап-х помещениях

    tempstruct.pov->NEDOG = query.value("t_gv1").toDouble();  //  Величина недогрева водопроводной воды в первой ступени подогревателя горячего водоснабжения, Град. C
    tempstruct.pov->KSR = query.value("uf").toDouble();  // Поправка коэффициента смешения
    tempstruct.pov->V = query.value("v").toDouble();  // Скорость ветра
    ...
    if (1) {
        tempstruct.pov->QMAX = query.value("hSourcePower").toDouble();
...
void InitSKTempStructure(QSqlQuery &query, COMMON_TEMP_STRUCTURE& tempstruct, COMMON_TEMP_STRUCTURE& tempstruct_norm, bool pov)
{
    if (pov) tempstruct.feat = SkkPov;
    else tempstruct.feat = SkkPon;

    delete tempstruct.skk;
    ...
    tempstruct.skk->THOR = query.value("tn_5").toDouble(); // Расчетная температура наружного воздуха
    tempstruct.skk->THK = query.value("tn_1").toDouble(); // Температура наружного воздуха конца отопительного сезона
    tempstruct.skk->TVR = query.value("tvn_r").toDouble(); // Расчетная температура воздуха внутри помещения


    tempstruct.skk->TAURP = query.value("t1_r").toDouble();  // Расчетная темп-ра сетевой воды в подающем трубопроводе
    tempstruct.skk->TAURO = query.value("t2_r").toDouble();  //      -"-                       в обратном
    tempstruct.skk->TAURS = query.value("t3_r").toDouble();  //      -"-                       после узла смешения

    tempstruct.skk->QOR = query.value("q_r").toDouble();  // Расчетная тепловая нагузка на отопление
    tempstruct.skk->QGW = query.value("q_gv").toDouble();   // Среднечасовая нагрузка горячего водоснабжения, Гкал/ч

    tempstruct.skk->TSMIN = query.value("t1_2r").toDouble();  // Температура нижней срезки  сетевой воды
    tempstruct.skk->TSMIN = 0; // Для скорректированного графика не бывает нижней срезки, сказала Надежда Ивановна (2022.01.19)
                              // Манюк стр 155,166



    tempstruct.skk->TSMAX = query.value("t1_4r").toDouble();  // Температура верхней срезки  сетевой воды
    tempstruct.skk->T2MIN = query.value("t2_2r").toDouble();  // Температура нижней срезки обратной воды

    tempstruct.skk->KGUP = query.value("g1").toDouble();  // Коэффициент гидравлической устойчивости подающего труб-да
    tempstruct.skk->KGUO = query.value("g2").toDouble();  // Коэффициент гидравлической устойчивости обратного труб-да

    long u = query.value("pr").toInt();
    ...
    tempstruct.skk->T2GW = query.value("t2_gv").toDouble(); // Темпер-ра сетевой воды начала водоразбора из обрат-го тр-да
    tempstruct.skk->TV = query.value("tx_r").toDouble();  // Температура холодной воды
    tempstruct.skk->TB = query.value("tvb_tr").toDouble();  // Требуемая температура воздуха внутри отап-х помещениях
    tempstruct.skk->TVRO = query.value("tg_r").toDouble();  // Температура горячей воды в точке водоразбора


    tempstruct.skk->KSR = query.value("uf").toDouble();  // Поправка коэффициента смешения
    tempstruct.skk->V = query.value("v").toDouble();  // Скорость ветра
  ...
  */
    ...
    tempstruct.skk->QMAX = query.value("hSourcePower").toDouble();
    //    atof(Value);  // Отношение максимального количества тепла отпускаемого ТЭЦ ????
                            // к расчетному теплу на отопление магистрали
...
CTempGraph::CTempGraph(QSqlDatabase & db, QSqlQuery &query, int id, bool standart)
{
    if (standart) {
        defaultLoadTempGraph(db, query, id);
    }
    else {
        ogLoadTempGraph(db, query, id);
    }
    
}

void CTempGraph::defaultLoadTempGraph(QSqlDatabase & db, QSqlQuery &query, int id) {
...
//    q = QString("SELECT ist.* FROM net.v_heatsources ist JOIN net.v_nodes n ON n.id=ist.nodeID WHERE n.id = %1").arg(id);
    ...
    if (query_exec(db, query, q)) {
        if (query.next()) {
            typ = query.value("graphTypeID").toInt();

            if (typ == 1 || typ == 0)  // О
            {
                InitOtoplTempStructure(query, tempstruct, tempstruct_norm);
            }
            if (typ == 3)  // П
            {
                InitPovTempStructure(query, tempstruct, tempstruct_norm);
            }

            if (typ == 2)  // "СВ"
            {
                InitSKTempStructure(query, tempstruct, tempstruct_norm, true);
            }
            if (typ == 4) // "СН"
            {
                InitSKTempStructure(query, tempstruct, tempstruct_norm, false);
            }
        }
...
//            change_group_start(db, "Пересчет ТГ");
            
            //    }
            QString SQL_string_D;
            QString TableName1 = "deployedTempGraphs";
//            long affected;

            q = QString("DELETE FROM %1 WHERE hSourceID=%2").arg(TableName1).arg(id);
            ...
            if (tempstruct.feat == Otop)
            {
                ...
                {
                    QString SQL_string = SQL_string_D;
                    s = QString("%1, %2, %3, %4, %5, %6, %7, %8)").arg(id).arg(tempstruct.array_ptr[0][i]).arg(tempstruct.array_ptr[1][i]).arg(tempstruct.array_ptr[2][i]).arg(tempstruct.array_ptr[3][i]).arg(tempstruct.array_ptr[4][i]).arg(tempstruct.array_ptr[5][i]).arg(tempstruct.array_ptr[6][i]);
                    SQL_string += s;
                    query_exec(db, query, SQL_string);
                }
            }
            if (tempstruct.feat == Pov)
            {
                for (long i = 0; i <= tempstruct.pov_data->n; i++)
                {
                    ...
                    s = QString("%1, %2, %3, %4, %5, %6, %7)").arg(id).arg(tempstruct.array_ptr[0][i]).arg(tempstruct.array_ptr[1][i]).arg(tempstruct.array_ptr[2][i]).arg(tempstruct.array_ptr[3][i]).arg(tempstruct.array_ptr[4][i]).arg(tempstruct.array_ptr[5][i]);
                    SQL_string += s;
                    query_exec(db, query, SQL_string);
                }
            }
            if ((tempstruct.feat == SkkPov) || (tempstruct.feat == SkkPon))
            {
                ...
                {
                    QString SQL_string = SQL_string_D;
                    s = QString("%1, %2, %3, %4, %5, %6)").arg(id).arg(tempstruct.array_ptr[0][i]).arg(tempstruct.array_ptr[1][i]).arg(tempstruct.array_ptr[3][i]).arg(tempstruct.array_ptr[2][i]).arg(tempstruct.array_ptr[4][i]);
                    SQL_string += s;
                    query_exec(db, query, SQL_string);
                }
            }
...
void CTempGraph::ogLoadTempGraph(QSqlDatabase & db, QSqlQuery &query, int id) {
...
{
    //QMessageBox::warning(this, "", "Срфк", MB_OK|MB_ICONINFORMATION);
}
...
bool CTempGraph::Calculate()
{
    ...
    if (TempStruct->feat == Otop)
    {
        ...
        {
            QMessageBox::warning(0, "", sError);
            return FALSE;
        }
        else
        {
            CalculateOT((*ot), (*data));
            delete [] TempStruct->array_ptr;
...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
void CTempGraph::AddError(UINT uID, QString& sError)
{
    sError += QString("Код ошибки %1 ").arg(uID);


    auto it = map_help.find(uID);
...
//----------------------------------------------------------------------------
bool CTempGraph::CheckInputOT(/*int cxema,*/ OTOP& ot, QString& sError)
{
    OTOP& tg = ot;
    bool Ok = true;
    //QString nl="\r\n\r\n";
...
//----------------------------------------------------------------------------
bool CTempGraph::CheckInputPOV(/*int cxema,*/ POV& ot, QString& sError)
{
...
//----------------------------------------------------------------------------
bool CTempGraph::CheckInputSK(/*int cxema,*/ SKK& sk, QString& sError)
{
    SKK& tg = sk;
    bool Ok = true;
