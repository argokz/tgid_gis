...
void createEmptyBorder(pugi::xml_node tblBorders, const wchar_t* tagBorder) {
    ...
    myBorder.append_attribute(L"w:sz").set_value(0);
    myBorder.append_attribute(L"w:space").set_value(0);
...
void createBorder(pugi::xml_node tblBorders, const wchar_t* tagBorder) {
    ...
    myBorder.append_attribute(L"w:sz").set_value(4);
    myBorder.append_attribute(L"w:space").set_value(0);
    myBorder.append_attribute(L"w:color").set_value(L"auto");
}
...
duckx::Table::insert_table(duckx::formatting_flag f) {
    ...
    if (f & duckx::align_center) {
        ...
        my_node2.append_attribute(L"w:w").set_value(5000);
...
//    wchar_t* text2 = _wcsdup(text.c_str());
    wchar_t* text2 = wcsdup(text.c_str());
...
duckx::Run& duckx::Paragraph::add_run(const wchar_t* text,
    ...
    if (m_size > 0) {
        meta.append_child(L"w:sz").append_attribute(L"w:val").set_value(m_size*2);
    }
    else {

        if (f & duckx::size_8)
            meta.append_child(L"w:sz").append_attribute(L"w:val").set_value(16);

        if (f & duckx::size_11)
            meta.append_child(L"w:sz").append_attribute(L"w:val").set_value(22);

        if (f & duckx::size_12)
            meta.append_child(L"w:sz").append_attribute(L"w:val").set_value(24);
    }
    ...
    if (true) {
        ...
        my_node.append_attribute(L"w:after").set_value(0);
    }
