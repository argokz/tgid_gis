...
//  bool isHide;
  ...
  QString getTableMySQL() const;
  QString getTableOut() const;
  QString getTableOutMySQL() const;

  QString getName() const;
  QString getNameParent() const;

  QString getNameFullFile() const;
  QString getNameFull() const;
  QString getNamePT() const;
  QString getNamePTfull() const;
...
//  return (CLine2 *)(s&0xfffffffcu);
  return (CLine2 *)(s&0xfffffffffffffffcu);

}

inline CNode2 *&where(const CLINE2 *l) 
{
  return bline(l)->_where[((UINT64)l)&0x1];
}
...
{
  return bline(l)->_where[1-(UINT64)l&0x1];
}
...
//  CGraph2 *m_graph;
  ...
  QString getKod(long id_kod) const;
...
public:
  ...
  int getKodId(const QString & kod);
  void getKodIds(const QString & kod, set<int> &set_id);

  CNode2 * findNodeName(const QString & kod, const QString & name, const char p);
  CLINE2 * findLineName(const QString & kod1, const QString & name1, const char p1, const QString & kod2, const QString & name2, const char p2);


  CNode2 * findNodeName_vn(const QString & kod_p, const QString & name_p, const char p_p, const QString & kod, const QString & name, const char p);
  CLINE2 * findLineName_vn(const QString & kod_p, const QString & name_p, const char p_p, const QString & kod1, const QString & name1, const char p1, const QString & kod2, const QString & name2, const char p2);
  ...
  std::list<CNode2 *> find_node_list(int parent_id, CFPoint pt, double r);
  std::list<CLINE2 *> find_line_list(int parent_id, CFPoint pt, double r);

  void init_find_node_2();
  CNode2 *find_node_2(int id);
  ...
  CLINE2 * findLineOut(const QString & tn, int id);
  CNode2 * findNodeOut(const QString & tn, int id);
  ...
  CNode2 * findPS1(const QString & name);
  CNode2 * findPS(const QString & name);
  ...
  void vyd_lines(set<int> & v);
  void vyd_nodes(set<int> & v);

  void vyd_lines_out(set<int> & v);

  void clean_out(set<int> & set_fileID);

  bool GetMarkRect(CFRect& rect);

  void remove_node(CNode2 &node);
  void remove_line(CLINE2 *line);
  void remove_fragments(set<int> & v);
  void remove_fragments(int fileID);
