...
bool readNodeNew(QSqlQuery& query, const ColIndex &C, CNode2* node2, int typ, CGraph2* m_graph, bool is_out)
{
    long id = query.value(C("id")).toInt();
    long fileID = query.value(C("fileID")).toInt();
    long internalNodeID = query.value(C("internalNodeID")).toInt();

    long externalCodeID = query.value(C("externalCodeID")).toInt();
    long externalSignID = query.value(C("externalSignID")).toInt();
    long podp = query.value(C("displaySign")).toInt();

    setCurrentExternalCode(fileID, externalCodeID);

    long consumerStateID = query.value(C("consumerStateID")).toInt();
    QString schemeNum = query.value(C("schemeNum")).toString();

    long connectID = query.value(C("connectID")).toInt();

    long nodeTypeID = query.value(C("nodeTypeID")).toInt();

    long id2 = query.value(C("id2")).toInt();

    QString name = query.value(C("externalNodeName")).toString();
    QString namePS = query.value(C("nodeName")).toString();

    double x = query.value(C("x")).toDouble();
    double y = query.value(C("y")).toDouble();

    x *= coef_xy;
    y *= coef_xy;

    double geoMarkTopTube = query.value(C("geoMarkTopTube")).toDouble();
    double geoMarkNodeArea = query.value(C("geoMarkNodeArea")).toDouble();

    double hz = query.value(C("hz")).toDouble();

    NAGR nagr;
    memset(&nagr, 0, sizeof(NAGR));

    if (typ == TIP_PO) {
        nagr.d_otopl_zco = query.value(C("calcHLdep")).toDouble();
        nagr.d_otopl_nco = query.value(C("calcHLindep")).toDouble();
        nagr.d_ventil_cv = query.value(C("calcHLventil")).toDouble();
        nagr.d_otopl_pr = query.value(C("calcHLparall")).toDouble();
        nagr.d_gv_pr = query.value(C("calcHLGVSparall")).toDouble();
        nagr.d_rez_pr = query.value(C("avgHLcompParall")).toDouble();
        nagr.d_otopl_sm = query.value(C("calcHLmix")).toDouble();
        nagr.d_gv_sm = query.value(C("calcHLGVSmix")).toDouble();
        nagr.d_rez_sm = query.value(C("avgHLcompMix")).toDouble();
        nagr.d_otopl_ps = query.value(C("calcHLconseq")).toDouble();
        nagr.d_gv_ps = query.value(C("calcHLGVSconseq")).toDouble();
        nagr.d_rez_ps = query.value(C("avgHLcompconseq")).toDouble();
        nagr.d_otopl_pw = query.value(C("calcHLpreON")).toDouble();
        nagr.d_gv_pw = query.value(C("calcHLGVSpreON")).toDouble();
        nagr.d_rez_pw = query.value(C("avgHLcompPreON")).toDouble();
        nagr.d_gv_op = query.value(C("avgHLGVSopenSysFlow")).toDouble();
        nagr.d_gv_oo = query.value(C("avgHLGVSopenSysRet")).toDouble();
        nagr.d_rez_op = query.value(C("avgHLcompOpen")).toDouble();
    }

    node2->node.pribor_ucheta = 0;

    if (typ == TIP_PR) {
        node2->node.pribor_ucheta = (query.value(C("WEmeteringDeviceID")) == 2);
        node2->node.nomDR = query.value(C("nomDR")).toInt();

        nagr.d_otoplz = query.value(C("calcHLdep")).toDouble();
        nagr.d_otopln = query.value(C("calcHLindep")).toDouble();
        //        nagr.d_otn_fs = query.value(C("relLoadFacade")).toDouble();
        nagr.d_otopl_tp = query.value(C("calcInternHD")).toDouble();
        nagr.d_ventil = query.value(C("calcHLventil")).toDouble();
        nagr.d_dolja_vent = query.value(C("expendHWpart")).toDouble();
        nagr.d_kondiz = query.value(C("avgHLcond")).toDouble();
        nagr.d_txz = query.value(C("avgHLcloseSys")).toDouble();
        nagr.d_txop = query.value(C("avgHLopenSysFlow")).toDouble();
        nagr.d_txoo = query.value(C("avgHLopenSysRet")).toDouble();
        nagr.d_gvop = query.value(C("avgHLGVSopenFlow")).toDouble();
        nagr.d_gvoo = query.value(C("avgHLGVSopenRet")).toDouble();
        nagr.d_gvpr = query.value(C("avgHLGVScloseParall")).toDouble();
        nagr.d_gvsm = query.value(C("avgHLGVScloseMix")).toDouble();
        nagr.d_gvps = query.value(C("avgHLGVScloseConseq")).toDouble();
        nagr.d_gvpw = query.value(C("avgHLGVSclosePreON")).toDouble();
    }

    long automDegID = query.value(C("automDegID")).toInt();
    ...
    node2->node.nomgP = id;
    node2->node.data.pihP = query.value(C("pihP")).toDouble();
    node2->node.data.tempP = query.value(C("tP1")).toDouble();

    node2->node.nomgO = id;
    node2->node.data.pihO = query.value(C("pihO")).toDouble();
    node2->node.data.tempO = query.value(C("tO1")).toDouble();

    if (typ == TIP_PR || typ == TIP_PO) {
        node2->node.qz = query.value(C("a15")).toDouble();
        node2->node.qP = query.value(C("a16")).toDouble();
        node2->node.qO = query.value(C("a17")).toDouble();

        if (node2->node.qz == 0 && node2->node.qP == 0 && node2->node.qO == 0) {
            node2->node.isZakr = query.value(C("isZakr")).toBool();
        }
        node2->node.nomPT = query.value(C("nomPT")).toInt();
    }
...
//    QInputDialog::getMultiLineText(nullptr, "", "", q);
    ...
    // Имена колонок разбираются один раз (см. cxema/col_index.h):
    // прежде каждое обращение по имени строило QSqlRecord и линейно
    // искало имя, и это стоило 12,3 с на 86 707 узлов.
    ...
    while (query.next()) {
        ...
        if (node2) {
            readNodeNew(query, C, node2, typ, m_graph);
