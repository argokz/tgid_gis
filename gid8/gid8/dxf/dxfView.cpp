...
int getFontFlag(int flags0)
{
    int flags = Qt::AlignLeft|Qt::AlignTop|Qt::TextDontClip;

    switch (flags0) {
    case 1:  flags = Qt::AlignLeft|Qt::AlignTop|Qt::TextDontClip; break;
    case 2:  flags = Qt::AlignCenter|Qt::AlignTop|Qt::TextDontClip; break;
    case 3:  flags = Qt::AlignRight|Qt::AlignTop|Qt::TextDontClip; break;
    case 4:  flags = Qt::AlignLeft|Qt::AlignBottom|Qt::TextDontClip; break;
    case 5:  flags = Qt::AlignCenter|Qt::AlignTop|Qt::TextDontClip; break;
    case 6:  flags = Qt::AlignRight|Qt::AlignBottom|Qt::TextDontClip; break;
    case 7:  flags = Qt::AlignLeft|Qt::AlignBottom|Qt::TextDontClip; break;
    case 8:  flags = Qt::AlignCenter|Qt::AlignTop|Qt::TextDontClip; break;
    case 9:  flags = Qt::AlignRight|Qt::AlignBottom|Qt::TextDontClip; break;
    }
...
/*
void cross(QPainter *pDC, int x, int y) {
...
QPoint screen_coord(cdxf* dxf, double x, double y, Transf transf)
{
    //  return QPoint(x*dxf->p25400/masx-m_bx, -y*p25400/masy-m_by);
    ...
    QPoint pt((int)(x2 * dxf->p25400 / dxf->masx - dxf->m_bx), (int)(-y2 * dxf->p25400 / dxf->masy - dxf->m_by));
...
//    int r = fabs(w * dxf->p25400 / dxf->masx) + 0.5;
    ...
    return (int) fabs(w * dxf->p25400 / dxf->masx) + 0.5;
}
...
void drawDxfanchor(generic* anchor, cdxf* dxf, QPainter* pDC, CRect m_rect, double m_bx, double m_by, double masx, double masy, Transf transf);
block_type* find_block(char* s, cdxf* dxf);


void drawDxfEnt(generic* en, cdxf* dxf, QPainter* pDC, CRect m_rect, double m_bx, double m_by, double masx, double masy, Transf transf)
{
    QPoint* p;
    int color;
    int n, j;
    ...
    case INSERT:
    {
        ...
        if (bl) {
            ...
            pDC->setPen(QPen(QColor(dxf_color(0)), 0.0 * screen_width(dxf, 1)));
//            CPen pen(PS_SOLID, 0.0 * screen_width(dxf, 1), dxf_color(0));

            QPoint p = screen_coord(dxf, in->ix, in->iy, transf);
            pDC->drawEllipse(p.x() - 1, p.y() - 1, 2, 2);

            if (0) {
                QPolygon p;
                p.push_back(screen_coord(dxf, bl->xmin, bl->ymin, tr2));
                p.push_back(screen_coord(dxf, bl->xmin, bl->ymax, tr2));
                p.push_back(screen_coord(dxf, bl->xmax, bl->ymax, tr2));
                p.push_back(screen_coord(dxf, bl->xmax, bl->ymin, tr2));
    ...
    case LWPOLYLINE:
    {
        ...
        for (j = 0; j < n; j++) p.push_back(screen_coord(dxf, pw->x[j], pw->y[j], transf));
//        if (pw->last_one) p[j++] = p[0];


        if (p.size()) {
            pDC->setPen(QPen(QColor(dxf_color(color)), 0.0 * screen_width(dxf, pw->w)));
//            CPen pen(PS_SOLID, 0.0 * screen_width(dxf, pw->w), dxf_color(color));
            pDC->drawPolyline(p);
        }
    }
    ...
    */
    ...
    case SPLINE:
    {
        ...
        for (j = 0; j < n; j++) {
            p.push_back(screen_coord(dxf, sp->x[j], sp->y[j], transf));
        }
        if (sp->last_one == 1) {
            //          types[j] = PT_BEZIERTO;
            //          p[j++] = p[0];
        ...
        if (j) {
//            CPen pen(PS_SOLID, 0.0 * screen_width(dxf, 0), dxf_color(color));
            pDC->setPen(QPen(QColor(dxf_color(color)), 0.0 * screen_width(dxf, 0)));
            pDC->drawPolyline(p);
//            PolyBezier(p, j);
        }
    }
    ...
    case HATCH:
    {
        ...
        for (j = 0; j < n; j++) {
            p.push_back(screen_coord(dxf, ht->x[j], ht->y[j], transf));
        }
        if (ht->last_one) {
//            p[j++] = p[0];
        }
...
//                    CPen pen(PS_SOLID, 0.0 * screen_width(dxf, w1), dxf_color(color));
                    pDC->setPen(QPen(QColor(dxf_color(color)), 0.0 * screen_width(dxf, w1)));
...
*/
            ...
            for (v = pl->vertex; v; v = v->next) {
                if (v->x != 0 || v->y != 0) {
                    p.push_back(screen_coord(dxf, v->x, v->y, transf));
                }
            }
            if (pl->last_one == 1) {
//                p[j++] = p[0];
...
//                CPen pen(PS_SOLID, 0.0 * screen_width(dxf, w1), dxf_color(color));
    ...
    case SOLID:
    {
        ...
        p.push_back(screen_coord(dxf, sl->x1, sl->y1, transf));
        p.push_back(screen_coord(dxf, sl->x2, sl->y2, transf));
        p.push_back(screen_coord(dxf, sl->x4, sl->y4, transf));
        p.push_back(screen_coord(dxf, sl->x3, sl->y3, transf));
        p.push_back(screen_coord(dxf, sl->x1, sl->y1, transf));
...
//        CPen pen(PS_SOLID, 0.0 * screen_width(dxf, 0), dxf_color(color));
    ...
    case LINE:
    {
        ...
        QPolygon p;
        p.push_back(screen_coord(dxf, lt->sx, lt->sy, transf));
        p.push_back(screen_coord(dxf, lt->ex, lt->ey, transf));

        layer_type* lyr = lt->lyr;

        if (lyr) {
//            CPen pen(PS_SOLID, 0.0 * screen_width(dxf, 0), dxf_color(lyr->color));
            pDC->setPen(QPen(QColor(dxf_color(lyr->color)), 0.0 * screen_width(dxf, 0)));
            pDC->drawPolyline(p);
        }
    ...
    case ELLIPSE:
    {
        ...
        pDC->setPen(QPen(QColor(dxf_color(color)), 0.0 * screen_width(dxf, el->w)));
//        CPen pen(PS_SOLID, 0.0 * screen_width(dxf, el->w), dxf_color(color));

        QPoint p1 = screen_coord(dxf, el->xmin, el->ymin, transf);
        QPoint p2 = screen_coord(dxf, el->xmax, el->ymax, transf);
        pDC->drawEllipse(p1.x(), p1.y(), p2.x()-p1.x(), p2.y()-p1.y());
    }
    break;
...
//        CPen pen(PS_SOLID, 0.0 * screen_width(dxf, cl->w), dxf_color(color));
        pDC->setPen(QPen(QColor(dxf_color(color)), 0.0 * screen_width(dxf, cl->w)));

        QPoint p1 = screen_coord(dxf, cl->xmin, cl->ymin, transf);
        QPoint p2 = screen_coord(dxf, cl->xmax, cl->ymax, transf);
...
//        CPen pen(PS_SOLID, 0.0 * screen_width(dxf, cl->w), dxf_color(color));
        ...
        QPoint p = screen_coord(dxf, cl->x, cl->y, transf);
...
/*
            ...
            if (te->font && !qstricmp(te->font, "Arial")) {
...
*/
            ...
            int flags = getFontFlag(te->justify);
...
//            pDC->SetTextAlign(TA_LEFT | TA_BASELINE);
            ...
            flags = Qt::AlignLeft|Qt::AlignBottom|Qt::TextDontClip;


            QPoint p1 = screen_coord(dxf, te->ix, te->iy, transf);
//            ::TextOutW(pDC->m_hDC, p1.x, p1.y, (LPCWSTR)us, nc);
            //          pDC->TextOut(p1.x, p1.y, us);
            //          pDC->Ellipse(p1.x-1, p1.y-1, p1.x+1, p1.y+1);

            pDC->setFont(font);
            pDC->drawText(p1.x(), p1.y(), QString::fromUtf8(te->text));
...
//            font.GetLogFont(&lf);
            ...
            if (mte->font && !qstricmp(mte->font, "Arial")) {
...
*/
            ...
            QFont font(mtext.font, h);


            int flags = getFontFlag(mte->gen_flags);


            QPoint p1 = screen_coord(dxf, mte->ix, mte->iy, transf);
...
void drawDxf(cdxf* dxf, QPainter* pDC, CRect m_rect, double m_bx, double m_by, double masx, double masy)
{
    ...
    QPoint pt1 = screen_coord(dxf, dxf->extent_x, dxf->extent_y, tr1);
    QPoint pt2 = screen_coord(dxf, dxf->extent_mx, dxf->extent_my, tr1);

//    CPen pen(PS_SOLID, 5, dxf_color(1));
    pDC->setPen(QPen(QColor(dxf_color(1)), 5));
    pDC->drawRect(pt1.x(), pt1.y(), pt2.x(), pt2.y());
    ...
    for (layout_type* lt = dxf->layout_anchor; lt; lt = lt->next) {
        ...
        QPoint pt1 = screen_coord(dxf, lt->xmin, lt->ymin, tr1);
        QPoint pt2 = screen_coord(dxf, lt->xmax, lt->ymax, tr1);
...
//        CPen pen2(PS_SOLID, 5, dxf_color(2));
        pDC->setPen(QPen(QColor(dxf_color(2)), 5));
        pDC->drawRect(pt1.x(), pt1.y(), pt2.x(), pt2.y());
...
void mtext_parser(char* s, Mtext& mtext)
{
    ...
    for (int i = 0, k = 0; s[k]; k++) {
        if (status == 0) {
            ...
            if (s[k] == ';' || s[k] == '}') {
                status = 2;
                strncpy_s(ff, &s[f0], k - f0); ff[k - f0] = 0;
