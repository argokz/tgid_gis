...
//#if MG_ENABLE_SSL
//  c[1]->ssl_if_data = nc->ssl_if_data;
//#endif
...
static const char *mg_version_header = "TGid-7/0.1";
...
*/
    mg_printf(nc,
              ...
              "Content-Length: %lu\r\n"
              "%sEtag: %s\r\n\r\n",
              current_time, last_modified, mime_type.p,
              (pd->file.keepalive ? "keep-alive" : "close"), (size_t) cl, range,
              etag);
...
//  mg_addenv(blk, "SERVER_SOFTWARE=%s/%s", "Mongoose", MG_VERSION);
  mg_addenv(blk, "SERVER_SOFTWARE=%s", mg_version_header);
