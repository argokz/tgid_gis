...
int cntr_in_M_POLYAREA(PLINE * poly, POLYAREA * outfst)
{
    ...
    assert1(poly != NULL, 32);
    assert1(outer != NULL, 33);
...
{
    assert1(ppl != NULL && ppl->contours != NULL, 35);
...
void InsertHoles(jmp_buf *e, POLYAREA * dest, PLINE **src)
{
    ...
    if (dest == NULL) throw 3; // (err_bad_parm); /* empty contour list */
