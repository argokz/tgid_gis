...
//    const QString datePattern = "([0-9]{,2})\\.([0-9]{,2})\\.([0-9]{,4})";
//    const QString datePattern = "([0-3]?[0-9]\\.[0-1]?[0-9]\\.[0-9]?[0-9]?[0-9]?[0-9])";
    const QString datePattern = "([0-3][0-9]\\.[0-1][0-9]\\.[0-9][0-9][0-9][0-9])";
...
void VariantDelegate::onActionTriggered()
{
  QMessageBox::warning(nullptr, tr("!"), "!");
}



QLineEdit *VariantDelegate::createLineEditor(QWidget *parent,
//        const QStyleOptionViewItem & /* option */,
...
//   QAction *myAction = lineEdit->addAction(QIcon(":/images/inf.png"), QLineEdit::TrailingPosition);
//    QAction *myAction = lineEdit->addAction(QIcon::fromTheme("edit-find"), QLineEdit::TrailingPosition);
//    connect(myAction, SIGNAL(triggered()), this, SLOT(onActionTriggered()));
...
QString VariantDelegate::displayText(const QVariant &value)
{
    ...
    case QMetaType::Double:
        {
            QString text = QLocale().toString(value.toDouble());
            text.replace("\u00A0", "");
            return text;
        }
