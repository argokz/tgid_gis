...
const double a = ellips_a1;
const double b = a*(1.-1./ellips_e);
...
void open_special_sk(const char* db_name)
{
    ...
    QString fn;
    fn = QString("%1kls/%2.sk").arg(argpath()).arg(db_name);
    QFile file(fn);
...
#if 0
void m2m_WGS84_42(SystemCoord& system_coord, double X1, double Y1, double &xx, double &yy, bool shift)
{
    double xxx, yyy;
...
*/
    ...
    if (false) {
        gis_convert1(yy, xx, xxxx, yyyy);
//        qDebug() << qSetRealNumberPrecision(15)  << X1 << " " << Y1 << " -> "  << yy << " " << xx << " -> " << xxxx << " " << yyyy;
        qDebug() << qSetRealNumberPrecision(15)  << X1 << " " << Y1 << " -> "  << yy << " " << xx << " -> " << xxxx << " " << yyyy;
/*
        yy = xxxx;
        xx = yyyy;
