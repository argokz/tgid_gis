...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format(
      ...
      r.relname, r.relname);
    EXECUTE format('INSERT INTO ref.%I SELECT * FROM public.%I',
                   r.relname, r.relname);

    IF EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = 'ref' AND table_name = r.relname
        AND column_name = 'id'
        AND data_type IN ('smallint', 'integer', 'bigint')
    ) THEN
      seq_name := r.relname || '_id_seq';
      EXECUTE format('CREATE SEQUENCE ref.%I', seq_name);
      EXECUTE format('SELECT max(id) FROM ref.%I', r.relname) INTO max_id;
      IF max_id IS NOT NULL THEN
        PERFORM setval(format('ref.%I', seq_name)::regclass, max_id, true);
      END IF;
      EXECUTE format(
        'ALTER TABLE ref.%I ALTER COLUMN id SET DEFAULT nextval(%L::regclass)',
        r.relname, format('ref.%I', seq_name));
      EXECUTE format('ALTER SEQUENCE ref.%I OWNED BY ref.%I.id',
                     seq_name, r.relname);
    END IF;
  END LOOP;
END
