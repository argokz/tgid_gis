...
PATCHES = [
    ...
    ('net_mode.py', "table_schema = 'public' AND table_name = 'nodes'",
     'is_net требует отсутствия базовой public.nodes. Одного наличия '
     'схемы net мало: в исходной БД она остаётся после пробной '
     'конвертации и устаревает, а чтение из неё даёт неполный расчёт.'),
    ('net_mode.py', "'consumptregulators': 'regulator_consumption'",
     'Пять продуктовых классов из sql/130 отсутствуют в карте конвертера, '
     'но sety должен читать их напрямую, а не искать legacy-таблицы.'),
    ...
    ('net_mode.py', 'def zn_query',
     'Узлы с заданным напором: запрос дважды соединялся с представлением '
     'nodes и стоил 857 мс — самый дорогой оператор прогона.'),
    ...
    ('read_gid.py', 'net_mode.node_query',
     'Подключение нативного чтения УЗЛОВ. Функция была написана, но '
    ...
    ('read_vnutr.py', 'net_mode.pt_node_query',
     ...
     'потребителей.'),
    ('read_tg.py', "q = f'''",
     'Запрос температурного графика обязан быть f-строкой: иначе ODBC '
     'получает литерал {net_mode.tbl(...)} и молча пропускает график.'),
    ('tg.py', "q = f'''",
     'Та же подстановка net-источников во втором загрузчике графика.'),
    ('net_mode.py', 'def consumer_update_targets',
     'Запись расчётных полей напрямую в объектные и extra-таблицы '
     'потребителей, без обновляемых compat-представлений.'),
    ('g2.py', 'net_mode.consumer_update_targets',
     'Запись рассчитанных нагрузок обобщённых потребителей в обе '
     'физические ветви новой модели.'),
    ('dross1.py', "consumer_update_targets('realconsumers')",
     'Запись диаметров дросселирующих органов реальных потребителей '
     'напрямую в net.'),
