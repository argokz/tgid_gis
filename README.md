...
4. **Оставшиеся запросы старого `gid8`** — после включения внешних SQL-файлов
   анализатор видит 696 имён отношений: 278 уже разрешаются в целевых схемах,
   3 остаются в `public`, 415 требуют отделить реальные запросы от временных
   таблиц, SQL Server DDL и уже удалённых подсистем. Начинать с `pipesections`,
   дефектов/ремонтов и паспортов.
