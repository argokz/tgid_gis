...
4. **Оставшиеся запросы старого `gid8`** — после включения внешних SQL-файлов
   анализатор видит 696 имён отношений: 279 уже разрешаются в целевых схемах,
   3 остаются в `public`, 414 требуют отделить реальные запросы от временных
   таблиц, SQL Server DDL и уже удалённых подсистем. Следующий законченный блок —
   дефекты, опрессовки и ремонты (`defect`, `opres`, `remont2`, `osmotr`).
5. **Функциональная матрица** — утвердить обязательные команды из
   [инвентаризации](docs/12-legacy-function-inventory.md).
6. **Запуск расчёта из Qt** — сам контур `sety` уже работает на чистых
