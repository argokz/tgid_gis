...
4. **Оставшиеся запросы старого `gid8`** — после включения внешних SQL-файлов
   анализатор видит 696 имён отношений: 324 уже разрешаются в целевых схемах,
   3 остаются в `public`, 369 требуют отделить реальные запросы от временных
   таблиц, SQL Server DDL и уже удалённых подсистем. Следующий законченный блок —
   электрические объекты и их карточки.
