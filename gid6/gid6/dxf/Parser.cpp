...
generic* line_parser(FILE* f, char* lyr, cdxf* dxf) {
    ...
    lt->xmin = min(lt->sx, lt->ex); lt->xmax = max(lt->sx, lt->ex);
    lt->ymin = min(lt->sy, lt->ey); lt->ymax = max(lt->sy, lt->ey);
...
generic* lwpline_parser(FILE* f, char* lyr, cdxf* dxf)
{
    ...
    while (1)
    {
        ...
        case N_10:
            ...
            pl->xmin = min(pl->xmin, pl->x[n_x - 1]);
            pl->xmax = max(pl->xmax, pl->x[n_x - 1]);
            break;
        case N_20:
            pl->y[n_y++] = atof(c) - s_dy;
            pl->ymin = min(pl->ymin, pl->y[n_y - 1]);
            pl->ymax = max(pl->ymax, pl->y[n_y - 1]);
            break;
        }
    }
...
generic* hatch_parser(FILE* f, char* lyr, cdxf* dxf)
{
    ...
    if (pl->n >= 1) {
        ...
        for (int i = 0; i < pl->n; i++) {
            pl->xmin = min(pl->xmin, pl->x[i]);
            pl->xmax = max(pl->xmax, pl->x[i]);
            pl->ymin = min(pl->ymin, pl->y[i]);
            pl->ymax = max(pl->ymax, pl->y[i]);
        }
    }
...
generic* vertex_parser(FILE* f, line_type* pl, char* lyr, cdxf* dxf) {
    ...
    if (v = (vertex_type*)calloc(1, sizeof(vertex_type)))
    {
        v->next = pl->vertex;
        pl->vertex = v;
        v->x = vv.x;
    ...
    pl->xmin = min(pl->xmin, v->x);
    pl->xmax = max(pl->xmax, v->x);
    pl->ymin = min(pl->ymin, v->y);
    pl->ymax = max(pl->ymax, v->y);
