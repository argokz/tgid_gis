...
void write_line(FILE* dxf, double x1, double y1, double x2, double y2, const char* layer, int h) {
    ...
    write_DXF(dxf, 10, &x1);
    write_DXF(dxf, 20, &y1);
    write_DXF_double(dxf, 30, 0.);
    write_DXF(dxf, 11, &x2);
    write_DXF(dxf, 21, &y2);
    write_DXF(dxf, 31, &zero);
}
...
void write_point(FILE* dxf, double x, double y, const char* layer, int h) {
...
void write_circle(FILE* dxf, double x, double y, double r, const char* layer, int h) {
...
int write_pline(FILE* dxf, int flags, double width1, double width2, const char* layer, int h) {
    int i;
    ...
    write_DXF_int(dxf, 66, 1);
    write_DXF_double(dxf, 10, 0.);
    write_DXF_double(dxf, 20, 0.);
    write_DXF_double(dxf, 30, 0.);
...
void write_vertex(FILE* dxf, double x, double y, double width1, double width2, double bulge, const char* layer, int h) {
...
void fillpoly(FILE* dxf, int n, double* x, double* y, const char* layer, int h) {
    int i;

    int h2 = write_pline(dxf, 1, 0., 0., layer, h);

    for (i = 0; i < n; i++) {
        write_vertex(dxf, x[i], y[i], 0., 0., 0., layer, h2);
    }

    write_seqend(dxf, layer, h);
}


void drawpoly(FILE* dxf, int n, double* x, double* y, const char* layer, int h) {
    int i;

    int h2 = write_pline(dxf, 0, 0., 0., layer, h);

    for (i = 0; i < n; i++) {
        write_vertex(dxf, x[i], y[i], 0., 0., 0., layer, h2);
    }

    write_seqend(dxf, layer, h2);
}


void write_mtext(FILE* dxf, double x, double y, const char* txt, double angle, double ht, int just, int h) {
    double xx;
...
void write_text(FILE* dxf, double x, double y, const char* txt, double angle, double ht, int just, const char* layer, int h) 
{
...
void write_text(FILE* dxf, double x, double y, const char* txt, double angle, double ht, int just, int h) 
{
    write_text(dxf, x, y, txt, angle, ht, just, "RED", h);
}
...
void write_insert(FILE* dxf, const char* name, double x, double y, int at, const char* layer, int h) 
{
