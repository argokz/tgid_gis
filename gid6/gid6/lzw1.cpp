...
namespace
{
    ...
    unsigned calculateByteMap(const std::vector<byte2>& input, byte2* byteMap)
    {
    ...
    struct CodeString
    {
        ...
        CodeString(byte2 newByte = 0, unsigned pI = ~0U):
    ...
    class Dictionary
    {
        ...
        std::vector<byte2> decodedString;
     ...
     public:
        ...
        void decode(unsigned oldCode, unsigned code,
                    std::vector<byte2>& outStream, const byte2* byteMap)
        {
    ...
    void LZW_Encode(const std::vector<byte2>& input,
                    std::vector<byte2>& encoded, unsigned maxBits)
    {
        assert(maxBits < 32);
        const size_t size = input.size();
        ...
        encoded.push_back(byte2(maxBits));
        encoded.push_back(byte2(byteMapSize));
        if(mapped)
            for(unsigned i = 0; i < 256; ++i)
                if(byteMap[i] < 255) encoded.push_back(byte2(i));
...
void LZWCodec::decode(const std::vector<byte2>& encoded,
                      std::vector<byte2>& decoded)
{
    decoded.clear();
