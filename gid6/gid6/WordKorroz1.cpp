...
int CGidrView::WordKorroz(const char *doc_name1, int y2, const char *path0) 
{
...
//    CString doc_name = "Индикатор_коррозии";
    CString doc_name = doc_name1;
...
//            CString OO = getReal(transl("Индикатор_коррозии"), transl("Оценка коррозионного процесса"), O);
            ...
            if (ID > ID_old) {
              ...
              row.insert_cell(nomer.AllocSysString(), fNormLeft);
              row.insert_cell(kod1.AllocSysString(), fNormLeft);
              row.insert_cell(name1.AllocSysString(), fNormLeft);
              row.insert_cell(kod2.AllocSysString(), fNormLeft);
              row.insert_cell(name2.AllocSysString(), fNormLeft);
              row.insert_cell(tr.AllocSysString(), fNormLeft);
              sum = 0;
            }
