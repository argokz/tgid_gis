    ...
    : CDialog()
{
    ...
    //  m_DialogTemplate.style = DS_SETFONT | DS_MODALFRAME | DS_FIXEDSYS | WS_POPUP | WS_VISIBLE | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME;

    m_DialogTemplate.style = WS_CAPTION | WS_SYSMENU | WS_VISIBLE | DS_SETFONT | WS_THICKFRAME;
        ...
        if (pDynDialogItemEx != NULL) {
            if (!pDynDialogItemEx->IsDlgItemSubclassed()) {
                if (!pDynDialogItemEx->CreateEx(this)) {
                    AfxMessageBox(_T("Failed to create DlgItem."), MB_OK|MB_ICONINFORMATION);
                }
                else if (pDynDialogItemEx->GetSafeHwnd() != NULL) {
                    TCHAR szClassName[100];
                    GetClassName(pDynDialogItemEx->GetSafeHwnd(), szClassName, 100);
...
BOOL CDynDialogEx::OnInitDialog()
{
    ...
    m_profile.Format("DynDialogEx\\%s", m_strCaption);
...
int CDynDialogEx::DoModal()
{
    ...
    if (hLocal != NULL) {
        ...
        if (pBuffer == NULL) {
            LocalFree(hLocal);
            AfxMessageBox(_T("CDynDialogEx::DoModal() : LocalLock Failed"), MB_OK|MB_ICONINFORMATION);
        }
        ...
        *(WORD*)pdest = m_wFontSize;            // font size
        ...
        AfxMessageBox(_T("CDynDialogEx::DoModal() : LocalAllock Failed"), MB_OK|MB_ICONINFORMATION);
        return -1;
    }
}
...
void CDynDialogEx::AddSystemButtons()
{
    ...
    rect.left += 55;
    rect.right += 55;
    AddDlgControl(BUTTON, _T("Копировать"), STYLE_BUTTON, EXSTYLE_BUTTON, &rect, NULL, IDC_COPY);
}
...
void CDynDialogEx::OnSave()
{
    CFileDialog fd(FALSE, _T("TXT"), "", OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_CREATEPROMPT | OFN_PATHMUSTEXIST, _T("TXT-файл (*.txt)|*.txt|"), this);
    ...
    if (fd.DoModal() == IDOK) {
        ...
        if (f) {
            ...
            CString v;
            edit->GetWindowText(v);
            fwrite(v, v.GetLength(), 1, f);
            fclose(f);
        }
    }
}
...
void ButtonsMoveH(CWnd* wnd, int cx, int cy, int height, int off)
{
    ...
    while (c) {
        ...
        if (_tcscmp(_T("Button"), szClassName) == 0)
        {
            ...
            wpi.rcNormalPosition.bottom = cy - off;
            wpi.rcNormalPosition.top = wpi.rcNormalPosition.bottom - height;
...
*/
    ...
    if (edit && ::IsWindow(edit->m_hWnd)) edit->MoveWindow(sz);
}

void CDynDialogEx::OnDestroy()
{
    WINDOWPLACEMENT wpi;
