...
#define MES_SELECTALL   _T("Выделить все")
...
void CPropertyGridInPlaceEdit::OnContextMenu(CWnd* pWnd, CPoint point)
{
    SetFocus();
    CMenu menu;
    ...
    if (chAkt) {
      ...
      menu.InsertMenu(9, MF_BYPOSITION , 1001,
              "Изменить Документ");
    }
...
BOOL CPropertyGridInPlaceEdit::OnCommand(WPARAM wParam, LPARAM lParam)
{
    CWnd* pOwner = GetOwner();
    ...
    case 1001:
      if (pOwner) {
          pOwner->PostMessage(WM_PG_MESSAGE, 0,  0L);
       }
       break;
    }
...
void CPropertyGridInPlaceEdit::setDoc(CString fun)
{
  chAkt = true;
}
