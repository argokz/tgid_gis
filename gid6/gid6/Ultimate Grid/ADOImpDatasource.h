...
#define UG_ADO_DS_SET_DELETEMODE  115
#define UG_ADO_DS_SET_EDITMODE_2  116
...
public:
  ...
  virtual int OpenSQL( CString sql, CString conStr, int type_of_net);
  virtual BOOL IsOpen();
  virtual int Close();
  ...
  virtual int GetNumColsReal();
  virtual int GetColName(int col,CString * string);
  virtual int GetColNameReal(int col,CString * string);
  ...
  void excel(list<int> &list_col);

  int ShowAll(int all);
  int Execute(const char *q);
  int Execute0(const char *q);
...
protected:
  ...
  // editing
  BOOL    m_bAllowDelete;    // Flag indicating if editing is enabled or not;
  ...
  int m_n_keys;
  COleVariant m_keyName;
  COleVariant m_keyValue;
  COleVariant m_keyName2;
  COleVariant m_keyValue2;
  ...
  int m_showAll;      // Показывать все строки
  ...
  int m_rows_all;
    ...
    std::list<CString> list_q;  // Список лишних запросов
