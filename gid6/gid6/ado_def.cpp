...
static map<int, int> map_ur; // Удельные расходы
...
{
  map<int, int>::const_iterator it = map_CurrentExternalCode.find(fileID);
...
{
  map<int, int>::const_iterator it = map_ur.find(fileID);
  if (it != map_ur.end()) {
    return it->second;
  }
