...
{
  if (demoversion) {
    m_warning.LoadString(IDS_WARNING2);
  }
  else {
    m_warning.LoadString(IDS_WARNING);
  }
  m_name.LoadString(AFX_IDS_APP_TITLE);

  if (demoversion) {
    m_name += " Демонстрационная версия";
  }
...
int get_ost_hasp();
  ...
  int ost = get_ost_hasp();

  if (ost > 0) {
      m_ostat.Format("%d", ost);
  }
...
//    SetDlgItemText(IDC_DATE, __DATE__);
  #endif //_MAC
...
void CAboutBox::OnContextMenu(CWnd* pWnd, CPoint point) 
{
