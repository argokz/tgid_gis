...
`EXCEPT`. Исходные таблицы возвращают 76 183 строки, объектный слой `net` —
76 161. Все 22 отсутствующие строки зарегистрированы в `net.line_orphan`:
это активные legacy-участки с нулевыми координатами одного из конечных узлов,
которые нельзя публиковать как корректные GIS-объекты до восстановления
геометрии. Иных расхождений нет.
...
`pipe_section` и UTF-8 CSV. В интерфейс входят только 76 161 канонический
объект; 22 строки `line_orphan` будут добавлены после восстановления их узлов.
