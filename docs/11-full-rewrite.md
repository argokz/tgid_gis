...
22. Перенос `onUtZakrAll «Отключённые участки»`: точное исходное условие
    отсутствия результата участка в `calc.ut_out`, фильтры, карточка линии и
    CSV по каноническим объектам `net` без чтения legacy-таблиц приложения.
23. Чистый расчётный контур `sety`: продуктовая БД без `public.nodes` и
    переходных `net.v_*`, прямое чтение объектных/extra-таблиц, результаты в
    `calc`, детерминированная сверка — [28-clean-calc-engine.md](28-clean-calc-engine.md).
...
3. Встроить готовый расчётный контур `net`/`ref`/`calc` в Qt: запуск,
   отмена, прогресс и показ диагностического журнала.
