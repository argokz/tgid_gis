...
## Шаг 98: проверка пустых столбцов, размер плиток, драйвер БД

### Пустые столбцы — данные не потеряны

Сверка по каждому столбцу с эталонной `almatygid`:

| Таблица | Колонок | Пусто сейчас | Было пусто и до | Потеряно |
|---|---|---|---|---|
| v_nodes | 44 | 10 | 7 | gis, idremoved, sync |
| v_linesobj | 26 | 5 | 2 | gis, idremoved, sync |
| v_heatpipesections | 151 | 41 | 41 | нет |
| v_realconsumers | 160 | 7 | 7 | нет |
| v_generalizedconsumers | 154 | 5 | 5 | нет |

Пользовательских данных не потеряно нигде. Единственные три имени —
`gis`, `idremoved`, `sync` — служебные поля репликации старой схемы.
41 пустой столбец из 151 в паспорте участка был пуст и в исходной базе.

### Плитки MapTiler: @2x только на плотном экране

Замер: `streets-v4` @2x — 94 КБ, обычная — 36 КБ (в 2,7 раза). На экран
1920x1080 нужно ~54 плитки, то есть смена масштаба тянет ~5 МБ вместо
~1,9 МБ. MapTiler отдаёт с ОДНОГО хоста (Google и OSM — с трёх
поддоменов), а Qt держит не более шести соединений на хост — отсюда
медленное появление карты при зуме.

`@2x` теперь запрашивается только при `devicePixelRatio > 1.2`;
принудительно — настройка `maps/maptiler_retina`.

### Драйвер БД: ODBC против родного

Обнаружено: приложение ходит в PostgreSQL через **QODBC**, строка с
родным `QPSQL` была закомментирована, хотя `qsqlpsql.dll` в сборку
разворачивается. ODBC добавляет слой на каждую строку: диспетчер
драйверов, psqlODBC, своё преобразование типов.

Выбор вынесен в настройку `db/driver` (`QPSQL` | `QODBC`), по умолчанию
прежний QODBC — молча менять драйвер в производственном приложении
нельзя. Родной проверен `--db-smoke`: подключение, верный search_path,
91 491 узел, 25 фрагментов.

Стоимость запросов таблиц (родной libpq, для сравнения): паспорт участка
151 колонка / 84 623 строки — 1,8 с; узлы 44/91 491 — 0,73 с. Первые 256
строк после выполнения — 5 мс, то есть модель Qt ленива и тянет данные
порциями.

## Шаг 99: MapTiler без @2x, все стили в JPEG

Заказчик: карты MapTiler грузятся слишком долго, убрать @2x со всех.

Убрано полностью, вместе с определением плотности экрана: решение
принято по результату работы, а не по характеристикам монитора, и пути
вернуть @2x в коде не осталось.

Заодно замерен формат на одной и той же плитке:

| Стиль | PNG | JPEG |
|---|---|---|
| streets-v4 | 36 КБ | **17 КБ** |
| topo-v4 | 37 КБ | **17 КБ** |
| openstreetmap | 36 КБ | **23 КБ** |
| hybrid-v4 | 43 КБ | **26 КБ** |

Все четыре стиля переведены на JPEG; `isJpeg` дополнен, иначе плитка
легла бы на диск с чужим расширением и кэш промахивался бы.

Проверено запуском с очищенным кэшем: 54 плитки, средний размер
**11 670 Б**, расширение `.jpg`. Против ~94 КБ у @2x PNG это в восемь
раз меньше — на экран 0,6 МБ вместо 5 МБ.

## Шаг 100: проводник геобазы — только то, что можно включить

Заказчик: в геобазе показано то, что на карте включить нельзя, галочка
ничего не даёт, названия групп непонятны, слои wgs84 не нужны.

Причина: после списка из `kls/gid2.lst` в дерево досыпались ВСЕ таблицы
классификатора, без разбора.

* слои без геометрии больше не показываются — признак берётся из самой
  БД (колонка типа `geometry`), список запрашивается один раз за сеанс.
  С геометрией 69 таблиц;
* представления `*_wgs84` скрыты — их 82, это те же объекты в другой
  системе координат, заведены для внешних ГИС;
* группы получили русские названия. Группа в дереве — это имя схемы, и
  показывалась она как есть: `net`, `ref`, `addr`. Теперь «Объекты
  тепловой сети», «Справочники», «Адресный план», «Эксплуатация:
  ремонты, дефекты, шурфы» и так далее.

Если спросить БД не удалось, фильтр не применяется: лучше показать
лишнее, чем спрятать нужное.

## Замер после шага 97

Открытие всех 25 фрагментов: было 33,8 с, стало **26,3 с**. Узлы
12,3 → 9,9 с, участки 21,3 → 16,2 с. Разбор колонок по индексу дал
около 20% на обоих этапах.

Прогон шёл ещё через **QODBC** — настройка `db/driver` в тот момент не
подхватилась. Родной `QPSQL` проверен `--db-smoke` и на реальном
открытии схемы пока не мерился; это остаётся крупнейшим невыясненным
рычагом, потому что ODBC добавляет слой преобразования на каждую строку.
